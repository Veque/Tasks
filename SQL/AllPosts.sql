...
В эту таблицу можно заводить теги с одинаковыми Name, но разными PostId, т.о. реализуется отношение много-ео-многим.
Отдельной таблицы для тегов я не делаю, потому что чаще всего в таких сценариях теги отдельно не заводяться.
...
Отдельную таблицу для тегов следует делать, если ими требуется управлять независимо от статей, если они могут существовать не прикрепленными к таблицам.
...
--WITH (NOLOCK) -- не блокировать запись в таблицу. Можно применить, если не страшно, что результат чтения будет иногда грязный.
	ON t.PostId = p.Id